 ...
 *  - Inicializace dat na GPU a spuštění kreslení
 *  \link task1 první úkol \endlink.
 * \image html images/task1.png "Výstup po prvním úkolu" width=5cm
 *  - Implementace transformace vrcholů pomocí vertex shaderu
 *  \link task2 druhý úkol \endlink.
 * \image html images/task2.png "Výstup po druhém úkolu" width=5cm
 *  - Implementace phongova osvětlení pomocí fragment shaderu
 *  \link task3 třetí úkol \endlink.
 * \image html images/task3.png "Výstup cvičení" width=5cm
 * Seznam všech úkolů naleznete zde \link todo.html \endlink.
 *
 * Ve cvičení se seznámíte se základními funkcemi OpenGL a napíšete
 * si jednoduchý kód ve jazyce GLSL.
 * Prerekvizita cvičení je 11. přednáška o osvětlování a základech OpenGL.
 * Cvičení taktéž kopíruje zadání projektu, ve kterém je úkol totožný,
 * ale nevyužívá se OpenGL.
 *
 * V projektu je přítomen i příklad vykreslení jednoho čtverce:
 * v \link quadExample Quad Example\endlink.
 * Tento příklad můžete využít pro inspiraci a návod jak napsat cvičení.
 * Všechny principy OpenGL, které budete ve cvičení používat jsou tam použity
 * (tvorba bufferů, tvorba vertex arrays objektu, indexování,
 * uniformní proměnné, vertex atributy, přeposílání dat mezi vertex shaderem
 * a fragment shaderem).
 *
 ...
 *
 * Typické grafické API (OpenGL/Vulkan/DirectX) je složeno ze 2 částí:
 * CPU a GPU strany.
 *
 ...
 * Sémantika atributu není pevně stanovena (atributy mají pouze
 * pořadové číslo - attribIndex) a je na každém programátorovi/modelátorovi,
 * jakou sémantiku atributu přidělí.
 * \image html images/primitive.svg "Vertex attributes" width=10cm
 *
 * \b Fragment je kolekce několika atributů (podobně jako Vertex).
 * Tyto atributy mají svůj typ a počet komponent.
 * Fragmenty jsou produkovány resterizací, kde jsou atributy fragmetů
 * vypočítány z vertex atributů pomocí interpolace.
 * Fragment si lze představit jako útržek původního primitiva.
 *
 * <b>Fragment atribut</b> je jedna vlastnost fragmentu
 * (podobně jako vertex atribut).
 *
 * <b>Interpolace</b> Při přechodu mezi vertex atributem
 * a fragment atributem dochází k interpolaci atributů.
 * Atributy jsou váhovány podle pozice fragmentu v trojúhelníku
 ...
 * Vertex Processor se obvykle stará o transformace vrcholů modelu
 * (posuny, rotace, projekce).
 * Jelikož Vertex Processor pracuje po vrcholech, je vhodný pro efekty
 * jako vlnění na vodní hladině, displacement mapping apod.
 * Vertex Processor má informace pouze o jednom vrcholu v daném čase
 * (neví nic o sousednostech vrcholů).
 ...
 * Shader má také vstupy, které zůstávají konstantní a nejsou závislé
 * na číslu invokace shaderu.
 * Shaderů je několik typů, v tomto projektu se používají pouze 2 -
 * vertex shader a fragment shader.
 * V tomto projektu jsou shadery reprezentovány pomocí standardních
 * Cčkovských funkcí.
 *
 ...
 * Jeho výstupní inteface je vertex, který vždy obsahuje proměnnou
 * gl_Position - pozici vertexu v clip-space.
 *
 ...
 * Do bufferů se ukládají vertex attributy vextexů modelů nebo indexy na
 * vrcholy pro indexované vykreslování.
 ...
 *
 * <b>Uniformní proměnná</b> je proměná uložená v konstantní paměti GPU.
 * Všechny programovatelné bloky zobrazovacího řetězce z nich mohou pouze číst.
 ...
 * funkcí jako je glUniform1f, glUniform1i, glUniform2f,
 * glUniformMatrix4fv apod. Uniformní proměnné jsou vhodné například pro
 * uložení transformačních matic nebo uložení času.
 *
 ...
 *
 * <b>Zobrazovací řetězec</b> je obvykle kus hardware na grafické kartě,
 * který se stará o vyreslování.
 ...
 * Pokud se spustí kreslení N vrcholů, je vertex puller spuštěn N krát
 * a sestaví N vrcholů.
 * Nad každým vrcholem je puštěn vertex shader.
 * Výstupem vertex shaderu je nový vrchol.
 * Blok sestavení primitiv "si počká" na 3 vrcholy z vertex shaderu
 * (pro trojúhelník) a vloží je do jedné struktury.
 ...
 * Poté následuje viewport transformace, která podělené vrcholy
 * transformuje do rozlišení obrazovky.
 ...
 * per-fragment operace (zjištění viditelnosti fragmentů podle hloubky
 ...
 * proměnnou.
 *
 * <b>Vertex atribut lokace</b> je číslo, které reprezentuje jeden vertex
 * atribut.
 *
 * <b>gl_VertexID</b> je číslo vrcholu, které je vypočítáno pomocí indexování
 * a pořadového čísla vyvolání vertex shaderu.
 *
 * <b>Indexované kreslení</b> je způsob snížení redundance dat s využitím
 * indexů na vrcholy.
 * \image html images/drawElements.svg "Difference between indexed and non-indexed drawing" width=10cm
 *
 *
 * \section glsl Jazyk GLSL
 * Jazyk GLSL slouží pro implementaci algoritmů (shader programů),
 * které běží na grafické kartě.
 ...
 * Interface obsahuje deklarace globálních proměnných, pomocí kterých shader
 * komunikuje s okolím.
 * Funkce main je vykonána nad každým vertexem/fragmentem.
 * Kvalifikátor proměnné "uniform" říká, že data proměnné budou uložena v
 * konstantní paměti GPU, která lze změnit z CPU.
 * \verbatim
 * uniform mat4 viewMatrix;
 * \endverbatim
 * Kvalifikátor "in" a "out" říkají, že data proměnné přijdou z předcházejícího
 * programovatelného bloku nebo vertex pulleru nebo
 * odejdou do následujícího programovatelného bloku nebo do
 * per-fragment operací.
 * Lokaci lze získat pouze z uniformních proměnných a "in" proměnných
 * vertex shaderu.
 ...
 *
 * Linux v CVT: pustit skript linux_create_makefile.sh - to vám vytvoří makefile
 * ve složce build
 * Postup:
 ...
 * <a href="http://www.fit.vutbr.cz/~imilet/shared/cmake-3.11.1-win64-x64.zip">
 * cmake na kazi
 * </a>, nebo z
 ...
 * Cvičení bylo testováno na Ubuntu 14.04, Ubuntu 16.04, Visual Studio 2013,
 ...
 * Dokáže generovat makefile pro Linux, mingw, solution file pro
 * Microsoft Visual Studio, a další.
 ...
 * Pod Linuxem si stáhněte SDL2 zdrojáky, zkompilujte je (s pomocí CMAKE)
 * a nainstalujte knihovnu.
 *
 *
 * \section spousteni Spouštění
 *
 * Cvičení je možné po úspěšném přeložení pustit přes aplikaci
 * <b>izg2017Lab6</b>.
 *
 *
