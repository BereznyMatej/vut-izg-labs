...
/* Initialization has to be done in the scope of function (Limitation of C),
 * see the main function.*/
S_Vector *control_points_up = nullptr;
S_Vector *curve_points_up = nullptr;
S_Vector *control_points_down = nullptr;
S_Vector *curve_points_down = nullptr;
...
void getLinePoints(const S_Vector *end_points, S_Vector *out_points)
{
	...
	{
		getLinePoints2d(
			point2d_vecGetPtr(end_points, i - 1),
			point2d_vecGetPtr(end_points, i),
			out_points
		);
	}
}
...
void drawLines(const S_Vector *end_points, const S_RGBA *color)
{
	...
	for (int i = 0; i < point2d_vecSize(lines_points); i++)
	{
		...
		putPixel(static_cast<int>(pt->x), static_cast<int>(pt->y), color);
	}
}
...
void drawCurvesWithConstruction(
	const S_Vector *ctrl_points, const S_Vector *crv_points
)
{
...
void onDraw()
{
	...
	/* Test, pokud kopirujeme rozdilnou velikost frame_bufferu
	 * a rozdilne pameti, musime pamet prealokovat */
...
void runAnimations()
{
	SDL_Surface *mario = SDL_LoadBMP("data/mario.bmp");
	IZG_ASSERT(mario);
	...
	for (int i = 0; i < point2d_vecSize(curve_points_up); i++)
	{
		...
		SDL_Rect dstrect;
		dstrect.x = static_cast<Sint16>(position_up->x - mario->w / 2.0);
		dstrect.y = static_cast<Sint16>(position_up->y - mario->h);
		SDL_BlitSurface(mario, nullptr, screen, &dstrect);
		dstrect.x = static_cast<Sint16>(position_down->x - mario->w / 2.0);
		dstrect.y = static_cast<Sint16>(position_down->y - mario->h);
		SDL_BlitSurface(mario, nullptr, screen, &dstrect);
...
{
	IZG_INFO(
		"Application loaded - IZG LAB 04 - Curves.\n"
...
int main(int argc, char *argv[])
{
	...
	/* Nastaveni okna */
	SDL_WM_SetCaption(PROGRAM_TITLE, nullptr);
