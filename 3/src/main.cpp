 ...
 *
 *      Ovladani programu:
 *          "Leva mys"      - Přidá nový bod k polygonu
...
const char *PROGRAM_TITLE = "IZG/2018 Lab 03";
...
// Nazvy i/o souboru
const char *outputImageName = "data/out.bmp";
...
void runTest(int test)
{
	...
	switch (test)
	{
		...
		case 2:
			...
			pinedaTriangle(
				points[0], points[1], points[2], COLOR_SALMON, color2
			);
			delete[] points;
			break;
 ...
 */
void copyBuffer(
	RGBA *srcColors, RGBA *dstColors, int srcWidth, int srcHeight,
	int dstX, int dstY, int dstWidth
)
{
	...
	{
		memcpy(
			dstColorNew + i * dstWidth,
			srcColors + i * srcWidth,
			srcWidth * sizeof(RGBA)
		);
	}
}
...
void runAllMakeTiled()
{
	...
	for (int y = 0; y < 2; ++y)
	{
		for (int x = 0; x < 5; ++x)
		{
			...
			// Copy bitmap
			copyBuffer(
				framebuffer, tiled, width, height, x * width, y * height, fw
			);
		}
	}
...
void onDraw()
{
	...
	// Test, pokud kopirujeme rozdilnou velikost framebufferu
		...
		switch (algorithm)
		{
			...
			case USE_PINEDA_TRIANGLE_FILL:
				...
				if (pointCount >= 3)
				{
					...
					pinedaTriangle(
						pointsTmp[0], pointsTmp[1], pointsTmp[2], color1, color2
					);
					delete[] pointsTmp;
					points.clear();
					isOld = false;
...
{
	IZG_INFO(
		"Application loaded - IZG LAB 03 - 2D area filling. Controls:\n\n"
		 "Right mouse click: \n"
