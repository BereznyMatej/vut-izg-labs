...
)
{
	...
	edgeFncValues[0] =
		...
		- (minX - v1.x) * edgeParams[0].deltaY;
	edgeFncValues[1] =
		(minY - v2.y) * edgeParams[1].deltaX
		- (minX - v2.x) * edgeParams[1].deltaY;
	edgeFncValues[2] =
		(minY - v3.y) * edgeParams[2].deltaX
		- (minX - v3.x) * edgeParams[2].deltaY;
	...
	for (int y = minY; y <= maxY; y++)
	{
		...
		for (int x = minX; x <= maxX; x++)
		{
			...
			currentEdgeFncValues[0] -= edgeParams[0].deltaY;
			currentEdgeFncValues[1] -= edgeParams[1].deltaY;
			currentEdgeFncValues[2] -= edgeParams[2].deltaY;
		}

		edgeFncValues[0] += edgeParams[0].deltaX;
		edgeFncValues[1] += edgeParams[1].deltaX;
		edgeFncValues[2] += edgeParams[2].deltaX;
	}
...
)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		edgeParams[i] = EdgeParam(p2.x - p1.x, p2.y - p1.y);
	}
