...
SDL_Surface *screen = nullptr;    // SDL surfage
S_RGBA *frame_buffer = nullptr;   // frame buffer (RGB values) memory pointer
...
///////////////////////////////////////////////////////////////////////////////
// line culling with Liang-Barsky algorithm right before rendering
// to frame buffer
...
{
	// detect changes
	if (screen == nullptr || resize->w != screen->w || resize->h != screen->h)
	{
...
void onMouseMotion(SDL_MouseMotionEvent *motion)
{
	...
	else if (motion->state & SDL_BUTTON(3))
	{
		...
		if (scene_move_z < MAX_SCENE_MOVE)
		{
			scene_move_z = MAX_SCENE_MOVE;
		}
	}
}
...
{
	IZG_INFO(
		"Application loaded - IZG LAB 05 - 3D Transformations. Controls:\n\n"
...
int main(int argc, char **argv)
{
	...
	if (!LoadModel(model))
	{
		IZG_ERROR("Error: Failed to read input model!");
	}
	...
	// force calling of onResize function - necessary for buffer
	// initialization of buffers
	SDL_ResizeEvent resize = {
		SDL_VIDEORESIZE, default_frame_w, default_frame_h
	};
	onResize(&resize);
