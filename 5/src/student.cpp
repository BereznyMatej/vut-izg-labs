...
// b - second vertex
// color - drawing color
void ProjectLine(const S_Coords &a, const S_Coords &b, const S_RGBA &color)
{
...
void ProjectTriangle(
	const S_Coords &a, const S_Coords &b, const S_Coords &c,
	const S_Coords &n,
...
)
{
	...
	// camera positon v is (0 - ax, 0 - bx, -CAMERA_DIST - a.z)
	S_Coords v(0.0 - aa.x, 0.0 - aa.y, (double) -CAMERA_DIST - aa.z);

	// cosa = (vx * nx + vy * ny + vz * nz) / (|v| * |n|)
	// denominator is always positove
	double numerator = v.x * nn.x + v.y * nn.y + v.z * nn.z;
	if (numerator <= 0) // triangle is not visible => do not render
	{
		return;
	}
...
{
	// iterate through all object triangles
...
void DrawScene()
{
	...
	// get mirror matrix (matrix before rotation of the first object)
	Matrix mirror = trGetMatrix();

	// draw object with red color
	// add translation and rotation of object
	double dx = obj_move_x * 0.015, dy = obj_move_y * 0.015;
	trTranslate(dx, dy, 0.0);
	double angleX = obj_rot_x * 0.01, angleY = obj_rot_y * 0.01;
	trRotateX(angleX);
	trRotateY(angleY);
	...
	// set mirror matrix (matrix before rotation of the first object)
	trSetMatrix(mirror);

	// invert X axis of mirrored object
	trScale(-1.0, 1.0, 1.0);

	// perform the same tranformations as transformations for the first object
	trTranslate(dx, dy, 0.0);
	trRotateX(angleX);
	trRotateY(angleY);

	// project and draw the mirrored object
