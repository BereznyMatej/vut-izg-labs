...
// (given matrix is multiplied by actual model matrix)
...
// multiply model matrix by translation matrix
// (object is moved for given distances)
...
void trTransformVertex(S_Coords &p2, const S_Coords &p)
{
	...
	p2.x =
		p.x * model[0][0] + p.y * model[1][0] + p.z * model[2][0] + model[3][0];
	p2.y =
		p.x * model[0][1] + p.y * model[1][1] + p.z * model[2][1] + model[3][1];
	p2.z =
		p.x * model[0][2] + p.y * model[1][2] + p.z * model[2][2] + model[3][2];
}
...
{
	// multiplication of vector (x,y,z,0) and transformation matrix
...
void trProjectVertex(int &u, int &v, const S_Coords &p)
{
	...
	u = int(xx + frame_width / 2.0);
	v = int(yy + frame_height / 2.0);
}
