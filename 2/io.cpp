...
)
{
	...
	/* Kontrola frame_bufferu, nasledne uvolneni a realokace
	...
	*height = image->h;
	...
	/* pixel po pixelu ulozime do frame_bufferu
	 * (lepsi memcpy, ale pro demonstraci PutPixel) */
	for (y = 0; y < *height; ++y)
	{
		for (x = 0; x < *width; ++x)
		{
...
)
{
	 ...
	 * (lepsi memcpy, ale pro demonstraci funkcnosti GetPixel) */
	for (y = 0; y < height; ++y)
	{
		for (x = 0; x < width; ++x)
		{
			S_RGBA color = getPixel(x, y);
			*((Uint32 *) image->pixels + y * width + x) =
				SDL_MapRGB(image->format, color.red, color.green, color.blue);
		}
	}
...
bool drawLinesFromFile(const char *filename)
{
	...
	for (std::string line; std::getline(source, line);)
	{
		...
		drawLine(
			(int) x1, (int) y1 + draw_offset, (int) x2, (int) y2 + draw_offset
		);
	}
	return true;
}
