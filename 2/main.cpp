...
{
	/* Pokud je pustene tlacitko, ulozime konecny bod cary a
	 * zavolame vykresleni */
	x_end = mouse->x;
	y_end = mouse->y;
...
{
	/* Pokud je pustene tlacitko, zistime polomer kruznice a
	 * zavolame vykresleni */
	x_end = mouse->x;
	y_end = mouse->y;

	int radius = int(sqrt(
		1.0 * (x_begin - x_end) * (x_begin - x_end)
			+ (y_begin - y_end) * (y_begin - y_end)
	));
...
{
	IZG_INFO(
		"Application loaded - IZG LAB 02 - Vector Objects Rasterization. Controls:\n\n"
